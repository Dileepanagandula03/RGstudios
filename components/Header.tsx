        ...
        <div className="container mx-auto px-6 flex justify-between items-center">
          ...
          <nav className="hidden lg:flex items-center space-x-10">
            {navLinks.map((link) => (
              ...
              >
                {link.isDropdown ? (
                  <div className="flex items-center gap-1 cursor-pointer py-2 text-[11px] uppercase tracking-[2px] text-[#2D3134] hover:text-[#E91E63] font-black transition-colors group">
                    <Link to={link.path} className="hover:text-[#E91E63]">{link.name}</Link>
                    ...
                    <div 
                      className={`absolute top-full left-0 w-64 transition-all duration-300 ${activeDropdown === link.name ? 'opacity-100 translate-y-0 scale-100 pointer-events-auto' : 'opacity-0 translate-y-2 scale-95 pointer-events-none'}`}
          ...
          {navLinks.map((link) => (
            <div key={link.name} className="flex flex-col">
              <div className="flex items-center justify-between border-b border-gray-100">
                <Link 
                  to={link.path} 
                  className="text-3xl font-bold text-[#2D3134] py-6 flex-grow group"
                ...
                {link.isDropdown && (
                  <button 
                    ...
                    className="p-6 text-[#E91E63]"
                  >
                    <ChevronDown size={24} className={`transition-transform duration-300 ${mobileDropdown === link.name ? 'rotate-180' : ''}`} />
                  </button>
                )}
              </div>
              
              {link.isDropdown && (
                <div className={`pl-6 bg-gray-50/50 flex flex-col transition-all duration-300 overflow-hidden ${mobileDropdown === link.name ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
                  {link.items?.map((item, idx) => (
                    <Link 
                      key={idx} 
                      to={item.path} 
